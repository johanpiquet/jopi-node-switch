    ...
    for (const file of files) {
        ...
        if (fileStat.isDirectory()) {
            ...
            try {
                ...
                if (packageJson.nodeSwitch)
                    foundPackages.push(filePath);
            }
            catch {
...
async function transformTo(serverType, dirPath, verbose) {
    ...
    for (const packagePath of packages) {
        ...
        if (nodeSwitch === true)
            nodeSwitch = {};
        if (!nodeSwitch.node)
            nodeSwitch.node = "dist";
        if (!nodeSwitch.bun)
            nodeSwitch.bun = "src";
        if (serverType === "restore") {
            if (json["nodeSwitch-default"])
                serverType = json["nodeSwitch-default"];
            if (!serverType)
                serverType = "node";
        }
        ...
        if (json.main !== originalJson.main)
            mustUpdate = true;
        else if (json.type !== originalJson.type)
            mustUpdate = true;
        if (!mustUpdate)
            continue;
        let output;
        ...
        if (verbose)
            console.log("[" + serverType + "] - Updating", packagePath);
...
//# sourceMappingURL=jopiswitch.js.map